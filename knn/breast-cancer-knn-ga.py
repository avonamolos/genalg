...
y_pred = pipeline.predict(X_test) # со ТЕСТ
...
# sol1, fit1, _ = ga.best_solution()
# print("best_solution fit:", fit1)

# best_fit_hist = max(ga.best_solutions_fitness)
# print("max best_solutions_fitness:", best_fit_hist)





# Овие седат тука ако затребаат ако нешто тргне наопаку.

# ON GENERATION (СТАРА) АКО ТРЕБА ДА ПРИНТАМ ПО КОЛКУ ГИ ЗЕМА 
...
#     elif gen_number >= 15 and gen_number <= 20:
#         print(
#             f"Generation {gen_number} | "
#             f"Best Fitness: {ga_instance.best_solution()[1]:.4f}"
#             f"\nSelected Features ({len(selected_features)}): "
